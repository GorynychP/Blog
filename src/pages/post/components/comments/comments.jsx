...
const CommentsContainer = ({ className, comments, postId }) => {
	...
	const userName = useSelector(selectorUserLogin);
	const dispatch = useDispatch();
	const requestServer = useServerRequest();
	const accessGuest = checkAccess([ROLE.GUEST], roleId);
	const onNewCommentAdd = (userName, userId, postId, content) => {
		dispatch(
			addCommentAsync(requestServer, userName, userId, postId, content),
		);
		setNewComment('');
	};
	return (
		<div className={className}>
			...
			<div className="comments">
				{comments
					? comments.map(({ id, author, published_at, content }) => (
							<Comment
								key={id}
								id={id}
								postId={postId}
